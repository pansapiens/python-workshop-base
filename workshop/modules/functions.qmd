...
3. Create your own function that does a simple mathematical operation - you might create an `add(a, b)` function that adds two numbers, or maybe function to calculate the hypotenuse of a right angle triangle, `pythagoras(a, b)`.
...
Here's our `year_to_csv` function (without comments and the docstring):
...
1. How could you modify the function `create_csvs_by_year` to output the CSV file for only one year ? Make sure you name your modified version something different.

2. The output from `year_to_csv` always starts with `'function_surveys'`. `Modify `year_to_csv` so that you can set the output filename to whatever you want. Name your new function `year_to_csv_at_path`. Eg, `def year_to_csv_file(year, all_data, filename):`. 
   Test your new function - does it work ?
...
def create_csvs_by_year(year, all_data):
    year_to_csv(year, all_data)
```
...
def year_to_csv(year, all_data, filename):
    surveys_year = all_data[all_data.year == year]
    surveys_year.to_csv(f"./data/{filename}")
```
